...
const StatsPanel = memo(function StatsPanel() {
  ...
  const netWorth = useMemo(() => {
    ...
    const plotsVal = plots.length * (REAL_ASSETS.find(a => a.id === 'tanah')?.baseCost ?? 0) * 0.7
      + plots.flatMap(p => p.slots).reduce((sum, s) => {
        const def = REAL_ASSETS.find(a => a.id === s.assetId);
        return sum + (def ? def.baseCost * 0.7 : 0);
      }, 0);
  ...
  }, [balance, portfolio, plots, realAssets, marketAssets]);
  ...
  const prestige = useMemo(() => {
    ...
    for (const p of PRESTIGE_TITLES) { if (netWorth >= p.min) current = p; }
    return current;
  }, [netWorth]);
    ...
    <>
      ...
      <div className="flex flex-wrap gap-1 mt-auto pt-2 border-t border-zinc-800 justify-center">
        {PRESTIGE_TITLES.map(p => (
          <img key={p.min} src={p.icon} alt={p.title}
            style={{ width: 20, height: 20, imageRendering: 'pixelated', opacity: netWorth >= p.min ? 1 : 0.15 }} />
        ))}
      </div>
    </>
  );
  ...
  return (
    <div className="flex h-full gap-0 font-mono" style={{ alignItems: 'flex-start' }}>
      ...
      <div className="flex flex-col gap-3 pr-3 border-r border-zinc-800"
        style={{ width: '20%', minWidth: 160, position: 'sticky', top: 0, alignSelf: 'flex-start', maxHeight: 'calc(100vh - 130px)', overflow: 'hidden' }}>
        <StatsPanel />
        <CollectButton />
      </div>
